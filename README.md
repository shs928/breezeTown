...
**F10（或手柄 Start）打开设置**：窗口/全屏、垂直同步、主音量、键位重绑（点击条目后按新键，Esc 取消；恢复默认一键还原）。设置持久化于 `user://settings.cfg`，键位改动在测试环境（`BREEZETOWN_SAVE_ROOT` 非空）不生效，保证自动检查的按键注入稳定。**手柄**（标准 Xbox 布局）：左摇杆移动、L3 快跑、A 交互、X 使用工具、B 关闭、Y 背包、十字键上地图/下换种子/左右缩放、LB/RB 循环工具、Back 吃口粮、Start 设置；连接手柄后打开面板会自动聚焦首个按钮，可用方向键+A 导航。
