...
- **POLISH-05 二轮：树冠/森林季色（2026-09-18）**：①**GLB 树（森林/成树）**：按导入材质名前缀（PineLeaf*/Broadleaf*）识别树冠，原色 × 季色乘子**原地变异共享材质**（一次生效全场景；树干 WarmBark 与果实 AppleVermilion 排除；原色表以"材质名@路径"字符串为键——Object 键会因实例释放失效）。②**代码树冠（樱花/幼树，合并进 Batch）**：树叶改挂专属 `#fffffe` 材质（与通用白分离）→ StaticGeometry 合并时自成独立批次 → 季节切换只改这一个共享材质的 albedo（顶点色 × 乘子），零遍历成本。③世界构建树在旧缓存中并入白色批次不可分离——材质变更触发指纹失效自动重建，构建链同步重烘。**效果**：秋季橡树金黄/松树橄榄金（forest 实拍）、冬季樱花转霜紫+常绿松挂霜、春/夏近原色；灌木仍常绿（材质色非顶点色，留后续）。证据：`work/art01/polish05b-{winter-trees,autumn-forest,winter-forest}.png`、四季 `polish05-season-{1,29,57,85}.png`；season 18 + indoor 199；回归 14 套 + smoke 全 PASS；导出包退出 0 + smoke PASS。
- **SOAK-01 长时间运行回归（2026-09-18）**：新增 `tests/soak_checks.gd`——模拟整年经营（`SOAK_DAYS` 默认 112=四季轮转，`SOAK_SEED` 可复现）：确定性步进推时间，穿插存读档往返（金币/天数完整性断言）、四室内进出、矿场进出、四区域传送（驱动森林分块流式与水域碰撞）、熔炉整周期（投铜→随时间加工→年末 FINISHED）、农田全年追踪。指标采样：静态内存/对象数/节点数（boot/每 14 天/末尾）。**结果（112 天）**：SOAK PASS，34 次操作零脚本错误，内存 367→394MB 平台期（有界），节点 6384→8659 平台期（森林分块流式收敛），存读档多次往返无损，退出码 0。**排错**：--script 桩环境里 main._process 不运行——机器 tick 与 NPC 日程须测试显式驱动（与逐帧行为等价的批处理）；Performance 节点数监视器名为 OBJECT_NODE_COUNT。
- **TRANSPORT-02 驿站马车（2026-09-18，PLAY-01 方案 A 用户已选定）**：新增 `data/carriage_db.gd`（六站锚点/偏移/时长矩阵/票价，免费教学线路农场↔镇区）与 `carriage_station.gd`（站牌+候车椅+停驻马车的低模表现、解锁状态、焦点解析、存档）。交互：走旁自动解锁（toast）+ E 呼叫面板（未解锁站显示"？？？"）；乘车 = 淡出淡入 + `_advance_world_time(矩阵时长)` + 扣 10 币 + 抵达站旁（新站解锁提示）。存档 `travel` 键。时长矩阵依实测距离标定（农场—镇 2、矿 4、港 2、农庄 4、湖 3；镇—矿 3 等，对称矩阵领域断言）。**排错**：湖畔站与老王、农庄站与玛尔妮的日程锚点重叠导致驿站焦点抢对话焦点（quest_checks 传话链 7 项失败暴露）——两站偏移移出 NPC 日程半径后恢复。证据：`work/art01/transport02-station.png`；carriage 25 项 PASS；回归 indoor 199/first_map 154/npc 42/quest 31/smoke 全 PASS。

## 下一批任务（按顺序执行）

| 顺序 / ID | 状态 | 具体工作 | 验收标准 |
|---|---|---|---|
| 1 · PLAY-01 | **方案 A 已实施（TRANSPORT-02）** | 马车站六站+计时长计费+踩点解锁+存档全部落地；C1（快跑时间减半）未实施、可作后续节奏微调。剩余：用户实机验收手感。 | 六站全部可达；存读档含解锁状态；smoke 回归通过——均已完成。 |
...
| 3 · POLISH-01 | **进行中** | 第一轮（天气）与第二轮（输入层）完成：雨雪粒子、降雨自动浇地、InputMap 动作层与手柄支持（键盘行为不变）。第一轮（天气）、第二轮（输入层+手柄）、第三轮（设置面板：显示模式/垂直同步/主音量/键位重绑，ConfigFile 持久化）完成。第一二三轮（天气/输入+手柄/设置面板）完成；BUILD-01 构建导出完成；POLISH-04 退出堆损坏排查完成——发布形态改 debug 模板导出（退出干净+性能同级），get_as_text 缺陷已修。季节地表两轮完成（冬雪+秋黄+环境层+树冠季色）。剩余：灌木季色、音效、长时间回归、上报引擎 issue。 | 场景/存档回归、目标设备性能与真人体验证据齐全后判定首图完成度。 |
