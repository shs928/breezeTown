...
| 3 · POLISH-01 | **进行中** | 第一轮（天气）与第二轮（输入层）完成：雨雪粒子、降雨自动浇地、InputMap 动作层与手柄支持（键盘行为不变）。第一轮（天气）、第二轮（输入层+手柄）、第三轮（设置面板：显示模式/垂直同步/主音量/键位重绑，ConfigFile 持久化）完成。第一轮（天气）、第二轮（输入层+手柄）、第三轮（设置面板）完成；BUILD-01 构建导出完成（单文件 exe+内置烘焙世界）。剩余：季节地表变化、音效、长时间回归。 | 场景/存档回归、目标设备性能与真人体验证据齐全后判定首图完成度。 |
