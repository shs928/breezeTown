...
更新：2026-09-16。本文登记当前代码、来源、重建方式与待验收项。它不是测试通过报告，也不表示地图画风已达到参考图质量。
...
**归属按用户确认：西南为玩家农场，东北为 NPC 农庄。** 原图东北的 “Your Farm” 标签仅是图片内容，不覆盖这一归属。玩家出生点由西南农舍的实际门前位置计算。旧 [world-map-landscape.png](../art/reference/world-map-landscape.png)保留为早期构图参考；[valley-map.png](../art/reference/valley-map.png)用于羊皮纸导览图风格，不再决定默认地图的地理位置。
